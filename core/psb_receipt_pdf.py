...
def _dollars_to_words(amount) -> str:
    """Convert dollar amount to words for 'The sum of ___ Dollars' line."""
...
def _build_service_row_amounts(service_record) -> dict:
    """Map receipt line items to DMV Fee and Fee for Service columns."""
    amounts = {
...
def _draw_labeled_field(pdf, x, y, width, label, value, label_size=8, value_size=9):
    """Template-style field: label, underline, value on the line."""
...
def _draw_receipt_payment_history_page(pdf, service_record, margin_x):
    """Payment ledger attachment (page 2)."""
...
def render_psb_service_receipt(pdf, service_record) -> None:
    """Draw page 1 — NY PSB receipt form; page 2 — payment history."""
    width, height = letter
    margin = 36
    ...
    receipt_num = str(service_record.receipt_number or "")
